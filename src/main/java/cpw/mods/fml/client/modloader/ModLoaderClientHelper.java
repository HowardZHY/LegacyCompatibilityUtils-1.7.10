...
public class ModLoaderClientHelper implements IModLoaderSidedHelper {
    ...
    public static void handleFinishLoadingFor(ModLoaderModContainer mc, Minecraft game) {
        ...
        try {
            FMLLog.finer("Requesting renderers from basemod %s", mc.getModId());
            mod.addRenderer(renderers);
            FMLLog.finer("Received %d renderers from basemod %s", renderers.size(), mc.getModId());
        } catch (Exception e) {
            FMLLog.severe( "A severe problem was detected with the mod %s during the addRenderer call. Continuing, but expect odd results", mc.getModId());
            e.printStackTrace();
        }
        ...
        for (Entry<Class<? extends Entity>, Render> e : difference.entriesOnlyOnRight().entrySet()) {
            FMLLog.finer("Registering ModLoader entity renderer %s as instance of %s", e.getKey().getName(), e.getValue().getClass().getName());
        ...
        for (Entry<Class<? extends Entity>, ValueDifference<Render>> e : difference.entriesDiffering().entrySet()) {
            FMLLog.finer( "Registering ModLoader entity rendering override for %s as instance of %s", e.getKey().getName(), e.getValue().rightValue().getClass().getName());
            RenderingRegistry.registerEntityRenderingHandler(e.getKey(), e.getValue().rightValue());
        }
        try {
            mod.registerAnimation(game);
        } catch (Exception e) {
            FMLLog.severe( "A severe problem was detected with the mod %s during the registerAnimation call. Continuing, but expect odd results", mc.getModId());
            e.printStackTrace();
        }
    }
    ...
    public ModLoaderClientHelper(Minecraft client) {
        ...
        keyBindingContainers = Multimaps.newMultimap(Maps.newHashMap(), new KeyBindingHandler());
    }
    ...
    public static class KeyBindingHandler implements Supplier<Collection<ModLoaderKeyBindingHandler>> {
