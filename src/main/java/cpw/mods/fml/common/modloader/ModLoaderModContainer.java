...
public class ModLoaderModContainer implements ModContainer {
    ...
    private void configureMod(Class<? extends BaseModProxy> modClazz, ASMDataTable asmData) {
        ...
        if (modConfig.exists()) {
            try {
                ...
                FMLLog.severe("Error occured reading mod configuration file %s", modConfig.getName());
                e.printStackTrace();
        ...
        try {
            for (ASMData dat : Sets.union(asmData.getAnnotationsFor(this).get("net.minecraft.src.MLProp"), asmData.getAnnotationsFor(this).get("MLProp"))) {
                if (dat.getClassName().equals(modClazzName)) {
                    try {
                        ...
                        FMLLog.fine("Found an MLProp field %s in %s", dat.getObjectName(), getModId());
                    } catch (Exception e) {
                        FMLLog.severe( "An error occured trying to access field %s in mod %s", dat.getObjectName(), getModId());
                        e.printStackTrace();
                    }
                }
            }
            for (ModProperty property : mlPropFields) {
                if (!Modifier.isStatic(property.field().getModifiers())) {
                    FMLLog.fine("The MLProp field %s in mod %s appears not to be static", property.field().getName(), getModId());
                    continue;
                }
                FMLLog.fine("Considering MLProp field %s", property.field().getName());
                ...
                try {
                    ...
                    FMLLog.finer("Configuration for %s.%s found values default: %s, configured: %s, interpreted: %s", modClazzName, propertyName, defaultValue, propertyValue, currentValue);
                    if (currentValue != null && !currentValue.equals(defaultValue)) {
                        FMLLog.finer("Configuration for %s.%s value set to: %s", modClazzName, propertyName, currentValue);
                        f.set(null, currentValue);
                    }
                } catch (Exception e) {
                    FMLLog.severe("Invalid configuration found for %s in %s", propertyName, modConfig.getName());
                    e.printStackTrace();
                    throw new LoaderException(e);
                } finally {
            ...
            if (!mlPropFound && existingConfigFound) {
                ...
                FMLLog.severe("Error trying to write the config file %s", modConfig.getName());
                e.printStackTrace();
                throw new LoaderException(e);
            }
        }
    ...
    public Object parseValue(String val, ModProperty property, Class<?> type, String propertyName) {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        try {
            ...
            GameRegistry.registerWorldGenerator(ModLoaderHelper.buildWorldGenHelper(mod), 1);
    ...
    public List<String> getOwnedPackages() {
        return ImmutableList.of("net.minecraft.src");
    }
